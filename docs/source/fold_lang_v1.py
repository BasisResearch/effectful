...
from effectful.ops.semantics import apply, call, coproduct, evaluate, fwd, handler, product, typeof, fvsof
from effectful.ops.syntax import Scoped, deffn, defop, defdata, defterm
...
def fold(
    ...
    def promote_add(add: Callable[[V, V], V], a: V, b: V) -> V:
        if isinstance(b, collections.abc.Generator) or isinstance(a, collections.abc.Generator):
            ...
            return {k: a[k] if k not in b else b[k] if k not in a else promote_add(add, a[k], b[k]) for k in set(a) | set(b)}
...
def fold_weighted(
    ...
    return functools.reduce(
        lambda a, b: a + [b],
        unfold_weighted(semiring, streams, body),
        []
    )
