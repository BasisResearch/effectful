...
def test_body_raw_value_arithmetic():
    ...
    x = defop(int, name="x")
    result = Sum.reduce({x: [1, 2, 3]}, x() + 10)
...
def test_body_raw_value_multi_stream():
    ...
    x = defop(int, name="x")
    y = defop(int, name="y")
    result = Sum.reduce({x: [1, 2], y: [10, 20]}, x() + y())
...
def test_body_raw_value_tuple():
    ...
    x = defop(int, name="x")
    result = Sum.reduce({x: [1, 2, 3]}, (x(), x() + 1))
    assert result == (6, 9)
...
def test_body_callable_no_args():
    ...
    x = defop(int, name="x")
    result = Sum.reduce({x: [1, 2, 3]}, lambda: x())
...
def test_body_callable_with_args():
    ...
    x = defop(int, name="x")
    result = Sum.reduce({x: [1, 2, 3]}, lambda a: x() + a)
    assert callable(result)
...
def test_body_mapping():
    ...
    x = defop(int, name="x")
    result = Sum.reduce({x: [1, 2, 3]}, {"a": x(), "b": x() + x()})
...
def test_body_mapping_nested():
    ...
    x = defop(int, name="x")
    result = Sum.reduce({x: [1, 2, 3]}, {"outer": {"inner": x()}})
...
def test_body_interpretation():
    ...
    x = defop(int, name="x")
    y = defop(int, name="y")
    result = Sum.reduce({x: [1, 2, 3]}, {y: lambda: x()})
