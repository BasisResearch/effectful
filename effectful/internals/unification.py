...
def infer_return_type(bound_sig: inspect.BoundArguments) -> type:
    """
    ...
    if (result_fvs | pattern_fvs) & concrete_fvs:
        raise TypeError("Cannot unify free type variables in pattern and concrete types")
    ...
    for name, param in sig.parameters.items():
        ...
        if freetypevars(bound_sig.arguments[name]):
            raise NotImplementedError(f"Parameter '{name}' cannot have free type variables")
    ...
    result_type: type = substitute(sig.return_annotation, subs)
    if freetypevars(result_type):
        raise TypeError("Return type cannot have free type variables after substitution")
    return result_type


def unify(
    pattern: type | types.GenericAlias | collections.abc.Mapping | collections.abc.Sequence,
    concrete: type | types.GenericAlias | collections.abc.Mapping | collections.abc.Sequence,
    ...
    if isinstance(pattern, typing.TypeVar):
        if pattern in subs and subs[pattern] != concrete:
            raise TypeError(f"Cannot unify {pattern} with {concrete} (already unified with {subs[pattern]})")
    ...
    elif isinstance(pattern, collections.abc.Mapping) and isinstance(concrete, collections.abc.Mapping):
        if pattern.keys() != concrete.keys():
            raise TypeError(f"Cannot unify {pattern} with {concrete}")
        for key in pattern:
            subs = unify(pattern[key], concrete[key], subs)
        return subs
    elif isinstance(pattern, collections.abc.Sequence) and isinstance(concrete, collections.abc.Sequence):
...
def substitute(typ: type | types.GenericAlias, subs: collections.abc.Mapping[typing.TypeVar, type]) -> type:
    """
    ...
    elif typing.get_args(typ) and typing.get_origin(typ) is not None:
        new_args = tuple(substitute(arg, subs) for arg in typing.get_args(typ))
        return typing.get_origin(typ)[new_args]
    else:
        return typ
