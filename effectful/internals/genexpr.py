...
def handle_return_value(state: ReconstructionState, instr: dis.Instruction) -> ReconstructionState:
    ...
    if state.stack and isinstance(state.stack[-1], ast.Constant) and state.stack[-1].value is None:
...
def handle_call_function(state: ReconstructionState, instr: dis.Instruction) -> ReconstructionState:
    ...
    arg_count = instr.arg
    if len(state.stack) >= arg_count + 1:
...
def handle_make_function(state: ReconstructionState, instr: dis.Instruction) -> ReconstructionState:
    ...
    if len(state.stack) >= 2:
        name = state.stack[-1]  # Function name (usually '<lambda>')
        code_obj = state.stack[-2]  # Code object
        new_stack = state.stack[:-2]
        
        # For lambda functions, try to reconstruct the lambda expression
        if isinstance(code_obj, ast.Constant) and hasattr(code_obj.value, 'co_code'):
            ...
            if (len(lambda_instructions) == 4 and
                ...
                lambda_instructions[2].opname.startswith('BINARY_') and
                ...
                if op_name in op_map:
                    ...
                    new_stack = new_stack + [lambda_ast]
                    return replace(state, stack=new_stack)
...
@ensure_ast.register(type(iter((1,))))
def _ensure_ast_iterator(value: Iterator) -> ast.AST:
    """Convert iterator to AST - special handling for iterators"""
...
def reconstruct(genexpr: GeneratorType) -> ast.GeneratorExp | ast.ListComp | ast.SetComp | ast.DictComp:
    """
    Reconstructs an AST from a generator expression's bytecode.
    
    Args:
        genexpr (GeneratorType): The generator object to analyze.
        
    Returns:
        An AST node representing the generator expression.
        Can be ast.GeneratorExp, ast.ListComp, ast.SetComp, or ast.DictComp.
    """
