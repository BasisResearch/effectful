...
class Encodable[T, U](ABC):
    ...
    def __init__(self, base: type[T], enc: type[U], ctx: Mapping[str, Any]):
        self.base = base
        self.enc = enc
        self.ctx = ctx
...
def _encodable_object[T, U](
    ...
    class BaseEncodable(Encodable[T, T]):
        def encode(self, value: T) -> T:
            return typing.cast(T, adapter.validate_python(value))

        def decode(self, encoded_value: T) -> T:
            return typing.cast(T, adapter.validate_python(encoded_value))

        def serialize(
            self, encoded_value: T
        ...
        def deserialize(self, serialized_value: str) -> T:
            ...
            return typing.cast(T, adapter.validate_json(serialized_value))
...
def _encodable_tuple[T, U](
    ...
    class TupleEncodable(Encodable[T, typing.Any]):
        def encode(self, value: T) -> typing.Any:
            ...
            return tuple(
                [enc.encode(elem) for enc, elem in zip(element_encoders, value)]
            )

        def decode(self, encoded_value: typing.Any) -> T:
            if len(encoded_value) != len(element_encoders):
                raise ValueError(
                    f"tuple length {len(encoded_value)} does not match expected length {len(element_encoders)}"
                )
            decoded_elements: list[typing.Any] = [
                enc.decode(elem) for enc, elem in zip(element_encoders, encoded_value)
            ]
        ...
        def serialize(
            ...
            if has_image:
                ...
                for enc, elem in zip(element_encoders, encoded_value):
                    result.extend(enc.serialize(elem))
                return result
            else:
                # Use base serialization for non-image tuples
                adapter = pydantic.TypeAdapter(encoded_ty)
                json_str = adapter.dump_json(encoded_value).decode("utf-8")
                return [{"type": "text", "text": json_str}]
    ...
    return typing.cast(Encodable[T, U], TupleEncodable(ty, encoded_ty, ctx))
...
def _encodable_list[T, U](
    ...
    class ListEncodable(Encodable[T, typing.Any]):
        def encode(self, value: T) -> typing.Any:
            if not isinstance(value, list):
                raise TypeError(f"Expected list, got {type(value)}")
            return [element_encoder.encode(elem) for elem in value]

        def decode(self, encoded_value: typing.Any) -> T:
            decoded_elements: list[typing.Any] = [
                element_encoder.decode(elem) for elem in encoded_value
            ]
