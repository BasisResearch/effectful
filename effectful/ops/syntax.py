...
class Scoped(Annotation):
    ...
    * :class:`Scoped` and :func:`defop` can also express more complex scoping semantics.
    ...
    def binding_sites(self, sig: inspect.Signature) -> set[str]:
        binding_sites = set()
        return_ordinal = self._get_param_ordinal(sig.return_annotation)
        for name, param in sig.parameters.items():
            param_ordinal = self._get_param_ordinal(param)
            if (
                Scoped._param_is_var(param)
                and param_ordinal <= self.ordinal
                and not param_ordinal <= return_ordinal
            ):
                binding_sites.add(name)
        return set(binding_sites)
...
class _BaseOperation(Generic[Q, V], Operation[Q, V]):
    ...
    def __init__(
        ...
        *,
        ...
        self._fvs_rule = self._build_fvs_rule()
            ...
            if typing.get_origin(param.annotation) is typing.Annotated:
                for anno in typing.get_args(param.annotation)[1:]:
                    if isinstance(anno, Scoped):
                        sites = anno.binding_sites(sig)
                        if len(sites) > 0:
                            binding_sites[name] = sites

        if len(binding_sites) == 0:
            return lambda *args, **kwargs: (
                tuple(frozenset() for _ in args),
                {k: frozenset() for k in kwargs},
            )

        def fvs_rule(*args, **kwargs):
            ...
            for name, sites in binding_sites.items():
                for site in sites:
                    if site in result_sig.arguments[name]:
                        result_sig.arguments[name] |= bound_sig.arguments[name]
    ...
    def __fvs_rule__(
        ...
        return self._fvs_rule(*args, **kwargs)
