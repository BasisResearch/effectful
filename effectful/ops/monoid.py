...
class _GroundRow(collections.abc.Mapping):
    ...
    def __init__(
        ...
        shape = self._row_shape(cprod_body)
        assert shape is not None
        (self._index, self._value), self._value_streams = shape
    ...
    @staticmethod
    def _row_shape(cprod_body) -> tuple[tuple[tuple, Term], Mapping] | None:
...
class ReduceGroundCartesianProduct(ObjectInterpretation):
    ...
    def _plate_folds(body, monoid) -> collections.abc.Set[Operation]:
        ...
        if not folds or not distributes_over(inner_monoid, monoid):
            return frozenset()
        return {var for fold in folds for var in typing.cast(Mapping, fold.args[1])}
