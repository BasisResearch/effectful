    ...
    def reduce(self, monoid: Monoid, body, streams):
        ...
        if isinstance(body, Term) and _is_monoid_plus(body.op):
            ...
            for factor in body.args:
                if (
                    ...
                    and not (set(typing.cast(Mapping, factor.args[1])) & set(absorbed))
                ):
                    nested_body, nested_streams = factor.args
                    absorbed |= typing.cast(Mapping, nested_streams)
        ...
        for stream_key, stream_body in streams.items():
            ...
            peeling = coproduct(
                ...
                typing.cast(
                    ...
                    | {
                        ...
                        for plate in typing.cast(Mapping, reduce.args[1])
                    },
                ),
            )
            ...
            try:
                ...
                peeled_bodies = typing.cast(
                    tuple,
                    handler(peeling)(evaluate)(
                ...
                for subst, inner_reduce in zip(peeled_bodies, row_inner_reduces):
                    inner_streams = typing.cast(Mapping, inner_reduce.args[1])
            ...
            renaming = {
                ...
                and row.survives(typing.cast(Operation, inner_plate_op))
            }
            unified = tuple(subst_body for (subst_body, _) in substituted)
            if renaming:
                unified = typing.cast(
                    tuple,
                    handler(typing.cast(Interpretation, renaming))(evaluate)(unified),
                )
            ...
            combined_factors = [
                inner_monoid.reduce(
                    ...
                    {
                        k: v
                        for (k, v) in typing.cast(Mapping, inner_reduce.args[1]).items()
                        if k != plate
                    },
                )
